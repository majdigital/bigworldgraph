        ...
        with self.input().open("r") as sentences_file:
            with self.output().open("w") as nes_file:
                for line in sentences_file:
                    ...
                    ner_tagged_sentence = self.ner_tagger.tag(tokenized_sentence)
                    serialized_sentence = serialize_ne_tagged_sentence(sentence_id, ner_tagged_sentence)
    ...
    def run(self):
        with self.input().open("r") as sentences_file:
            with self.output().open("w") as dependency_file:
                for line in sentences_file:
                    sentence_id, sentence = line.split("\t")
                    parsed_sentence = self.dependency_parser.raw_parse(sentence)
                    serialized_tree = serialize_dependency_parse_tree(sentence_id, parsed_sentence)
...
if __name__ == "__main__":
    ...
    luigi.build([OpenRelationExtractionTask()], local_scheduler=True)
